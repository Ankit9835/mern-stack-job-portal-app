...
import { DISPLAY_ALERT,CLEAR_ALERT,REGISTER_USER_BEGIN,
    ...
    REGISTER_USER_ERROR,LOGIN_USER_BEGIN,
    LOGIN_USER_SUCCESS,LOGIN_USER_ERROR,
    SETUP_USER_BEGIN,SETUP_USER_SUCCESS,SETUP_USER_ERROR,
    TOGGLE_SIDEBAR,LOGOUT_USER, UPDATE_USER_BEGIN,UPDATE_USER_SUCCCESS,
    UPDATE_USER_ERROR,HANDLE_CHANGE,CLEAR_VALUES, CREATE_JOB_BEGIN, CREATE_JOB_SUCCESS, CREATE_JOB_ERROR,
    GET_JOB_BEGINS,GET_JOB_SUCCESS,SET_EDIT_JOB,DELETE_JOB_BEGIN,EDIT_JOB_BEGIN,EDIT_JOB_SUCCESS,EDIT_JOB_ERROR, SHOW_STATS_SUCCESS,
    SHOW_STATS_BEGIN,CLEAR_FILTER,CHANGE_PAGE_NUMBER
} from './actions'
  ...
  authFetch.interceptors.response.use(
    ...
    const updateUser = async (currentUser) => {
        ...
        try{
            ...
            const {user,token,location} = data
            dispatch({type:UPDATE_USER_SUCCCESS, payload:{user,location,token}})
            addUserToLocalStorage({ user, location, token });
        } catch (error){
            if(error.response.status === 401){
                logoutUser()
    ...
    const showStats = async () => {
        ...
        try{
            ...
            dispatch({type:SHOW_STATS_SUCCESS, payload:{stats:data.defaultStats,monthlyApplication:data.monthlyApplications}})
        } catch(error){
            console.log(error.response)
        }
        clearValues()
    ...
    return (
        ...
        logoutUser,updateUser,handleChange,clearValues,createJob,getJobs,setEditJob,deleteJob,editJob,showStats,clearFilters,changePage}}>
            {children}
        </AppContext.Provider>
    )
}
